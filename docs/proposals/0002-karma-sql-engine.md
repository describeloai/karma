...
| **4→1** ✅ | **Front: SQL → Substrait with our rules** — `KarmaSession::run(sql) → {columns, types, rows}`: a lazy resolver-backed catalog (FQN `catalog.schema.table`), our function registry (`get_json_object`, `approx_count_distinct`), auto-LIMIT, tenancy-by-resolver; executes through the Substrait contract with Karma pruning; returns explicit columns+types | Karma | **"Our vocabulary" as code** (`crates/karma-sql`: `session`/`catalog`/`udf`). Tested over an in-memory resolver + object store: FQN==bare, pruning fires end-to-end, UDFs bind through Substrait, columns typed |
| 5 | **Seam swap in Carbon** — route calls the Karma engine at `translateQuery→execute`, gated by the freshness oracle, PG fallback | Carbon | Deletes the shim — **next** |
